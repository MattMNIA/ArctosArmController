...
def nb_go_home(self):
    ...
    Note: Same as the "CanID" option on screen.
    """
    logging.info(f"Starting nb_go_home for servo {self.can_id}")
    ...
    try:
        ...
        logging.info(f"Servo {self.can_id} initial homing status: {rslt}")
    except ValueError:
        logging.error(f"Servo {self.can_id} received invalid homing status value {status_int}")
...
def b_go_home(self):
    ...
    Raises:
        can.CanError: If there is an error in sending the CAN message.
    """
    logging.info(f"Starting b_go_home for servo {self.can_id}")
    self._homing_status = GoHomeResult.Unkown  # Reset initial state
    rslt = self.nb_go_home()
    
    if rslt.value != GoHomeResult.Start.value:
        logging.error(f"Servo {self.can_id} failed to start homing, status: {rslt}")
        return rslt
        
    status = self.wait_for_go_home()
    logging.info(f"Servo {self.can_id} completed homing with status: {status}")
    return status


def wait_for_go_home(self, timeout=None):
    """
    Waits until the go home procedure completes.

    Args:
        timeout (float, optional): Maximum time to wait in seconds. Uses self.MAX_HOMING_TIME if not specified.
    ...
    """
    logging.info(f"Beginning wait_for_go_home for servo {self.can_id}, initial status: {self._homing_status}")

    if self._homing_status == GoHomeResult.Unkown:
        logging.error(f"Servo {self.can_id} homing status is Unknown, likely not started properly")
        raise calibration_not_running("")

    max_time = timeout if timeout is not None else self.MAX_HOMING_TIME
    start_time = time.perf_counter()
    last_status = self._homing_status
    while (time.perf_counter() - start_time < max_time) and self._homing_status == GoHomeResult.Start:
        time.sleep(0.1)  # Small sleep to prevent busy waiting
        if self._homing_status != last_status:
            logging.info(f"Servo {self.can_id} homing status changed: {last_status} -> {self._homing_status}")
            last_status = self._homing_status

    logging.info(f"Servo {self.can_id} completed waiting, final status: {self._homing_status}")
    
    # Check for success/failure 
    if self._homing_status == GoHomeResult.Success:
        logging.info(f"Servo {self.can_id} homing succeeded")
    elif self._homing_status == GoHomeResult.Fail:
        logging.warning(f"Servo {self.can_id} homing failed")
    else:
        elapsed = time.perf_counter() - start_time
        logging.error(f"Servo {self.can_id} homing timed out after {elapsed:.2f}s with status: {self._homing_status}")
        raise go_home_timeout_error("Maximum homing time reached")
